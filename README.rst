...
site. This is easy to understand when the content is e.g. an article, but
...
(e.g. `username`) are always visible on the registration form and cannot be
removed.

You can select a subset of the displayed fields to be required. Fields which
are absolutely required (e.g. `username`) cannot be set to be optional. For
...
change this setting. An exception is raised if you attempt to change this
...
`Widget` is the most complex. It is used when each item can be interactive, e.g.
...
view, e.g. `contact-us`.  The vocabulary is all the named views in the Django
...
can all be different per layer. This enables optimal support for different
...
If e.g. the template my_page.html is not found in the web layer then it falls
